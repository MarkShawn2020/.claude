...
check_git_relative_path_update () {
	if [[ -n "$ZSH_GIT_RELATIVE_PATH_REPO" ]]
	then
		local latest_version=$(curl -s "$ZSH_GIT_RELATIVE_PATH_REPO/raw/main/package.json" | grep -o '"version": "[^"]*"' | cut -d'"' -f4) 
		if [[ -n "$latest_version" && "$latest_version" != "$ZSH_GIT_RELATIVE_PATH_VERSION" ]]
		then
...
get_git_relative_path () {
	...
	if [[ -n "$workspace_root" && $current_path == $workspace_root* ]]
	then
...
ggg_2024-11-14 () {
	...
	process_repo () {
		...
		echo_start "$indent" "Processing repository: $repo_name"
		pushd "$repo_path" > /dev/null || return
		git submodule foreach --quiet 'echo "$path"' | while read -r submodule
		do
			process_repo "$submodule" "$submodule" "$indent$INDENT_CHAR"
		done
		if [[ -n $(git status --porcelain) ]]
...
log_debug () {
	[ "$LOG_LEVEL" -ge 4 ] && echo "[DEBUG][$SCRIPT_NAME] $*" >&2
}
log_error () {
	[ "$LOG_LEVEL" -ge 1 ] && echo "[ERROR][$SCRIPT_NAME] $*" >&2
}
log_info () {
	[ "$LOG_LEVEL" -ge 3 ] && echo "[INFO][$SCRIPT_NAME] $*" >&2
}
log_warn () {
	[ "$LOG_LEVEL" -ge 2 ] && echo "[WARN][$SCRIPT_NAME] $*" >&2
}
md2pdf () {
	echo "== started md2pdf =="
	input_file="" 
...
EOF
	...
	pandoc "$temp_file" -o "$output_file" -f markdown+wikilinks_title_after_pipe --template eisvogel -t pdf --pdf-engine=xelatex -V mainfont="Songti SC" -V CJKmainfont="Songti SC"
	rm "$temp_file"
...
wrap_prompt_with_git_path () {
	...
	case "$ZSH_THEME" in
		(robbyrussell) [[ -n "$ZSH_GIT_PATH_DEBUG" ]] && debug_log "Handling robbyrussell theme"
			PROMPT='%(?:%{$fg_bold[green]%}➜ :%{$fg_bold[red]%}➜ ) %{$fg[cyan]%}$(get_git_relative_path)%{$reset_color%} $(git_prompt_info)'  ;;
		(avit) [[ -n "$ZSH_GIT_PATH_DEBUG" ]] && debug_log "Handling avit theme"
			_current_dir="%{$fg_bold[blue]%}$(get_git_relative_path %3~)%{$reset_color%} "  ;;
		(agnoster) [[ -n "$ZSH_GIT_PATH_DEBUG" ]] && debug_log "Handling agnoster theme"
			prompt_dir () {
...
alias -- _='sudo '
alias -- cc=claude
alias -- ccc='cd ~/.claude && claude --add-dir ../__core__'
alias -- egrep='grep -E'
alias -- fgrep='grep -F'
alias -- g=git
alias -- ga='git add'
...
export PATH=/Users/mark/Library/pnpm\:/opt/homebrew/anaconda3/bin\:/Users/mark/.nvm/versions/node/v22.15.1/bin\:/opt/homebrew/bin\:/opt/homebrew/sbin\:/usr/local/bin\:/System/Cryptexes/App/usr/bin\:/usr/bin\:/bin\:/usr/sbin\:/sbin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin\:/opt/X11/bin\:/Library/Apple/usr/bin\:/Library/TeX/texbin\:/usr/local/go/bin\:/Users/mark/.cargo/bin\:/Applications/iTerm.app/Contents/Resources/utilities
